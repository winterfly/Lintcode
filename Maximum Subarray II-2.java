public class Solution {
    ...
    public int maxTwoSubArrays(ArrayList<Integer> nums) {
        ...
        // ! Note that we only need to use one variable to track left, so is right.
        ...
        for (int i = 1; i < n; i++) {
            left = Math.max(left + nums.get(i), nums.get(i));
            leftGlobal[i] = Math.max(leftGlobal[i-1], left);
        }
        int right = nums.get(n-1);
        rightGlobal[n-1] = nums.get(n-1);
        for (int i = n-2; i >= 0; i--) {
            right = Math.max(right + nums.get(i), nums.get(i));
            rightGlobal[i] = Math.max(rightGlobal[i+1], right);
        }
